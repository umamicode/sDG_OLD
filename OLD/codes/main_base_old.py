    ...
    #PACS  
        ...
        if backbone == 'custom':
            ...
            if optimizer == 'adam':
                ...
                cls_opt = optim.SGD(cls_net.parameters(), lr=0.001, momentum=0.9, nesterov=True, weight_decay=5e-4)
        elif backbone in ['vit']:
            #ViT_L_32_Weights
            encoder = get_resnet(backbone, pretrained) # Pretrained Backbone default as True
            n_features = encoder.heads.head.in_features
            output_dim= 7
            cls_net= vit_net.ConvNet(encoder, projection_dim, n_features, output_dim).cuda() #projection_dim/ n_features
            if optimizer == 'adam':
                cls_opt = optim.Adam(cls_net.parameters(), lr=lr)
            elif optimizer == 'sgd':
                cls_opt = optim.SGD(cls_net.parameters(), lr=lr, momentum=0.9, nesterov=True, weight_decay=5e-4)
        ...
        if lr_scheduler == 'cosine':
            ...
            scheduler = optim.lr_scheduler.StepLR(cls_opt, step_size= int(epochs * 0.8))
